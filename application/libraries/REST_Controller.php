...
class REST_Controller extends CI_Controller {
	...
	public function __construct()
	{
        ...
        $arr = array();
				$this->request->format and $this->request->body = parse_str($input, $arr);
        
        $json_data = json_decode($input);
        if (is_object($json_data)) {
          $arr = get_object_vars($json_data);
        }
        
				$this->_post_args = array_merge($this->_post_args, $arr);
				break;
	...
	public function option($key = NULL, $xss_clean = TRUE)
	{
	  if ($key === NULL)
		{
			return $this->_args;
